...
interface getTopThreePostsProps {
...
export interface getTopThreePostsResponse {
  postID: string
  userID: string
  admID: string
  admMessage: string
...
export async function getTopThreePosts({
  userID,
}: getTopThreePostsProps): Promise<getTopThreePostsResponse[]> {
  const response = await api.get<apiResponse[]>(
    `/Postagem/UltimasPostagensRealizadasPorUsuario/${userID}`,
  )

  const data: getTopThreePostsResponse[] = response.data.map((item) => ({
